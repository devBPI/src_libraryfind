  ...
  def fork_it(options)
    ...
    child = fork do
      begin
        ...
        @@logger.debug "spawn> PID = #{Process.pid}: close ressource"
        Spawn.close_resources
        # get a new connection so the parent can keep the original one
        @@logger.debug "spawn> PID = #{Process.pid}: spawn reconnect"
        ActiveRecord::Base.spawn_reconnect
        
        # run the block of code that takes so long
        @@logger.debug "spawn> PID = #{Process.pid}: execute yield"
        yield
        
      rescue => ex
        @@logger.error "spawn> Exception in child[#{Process.pid}] - #{ex.class}: #{ex.message}"
        @@logger.error "spawn> Exception in child[#{Process.pid}] - #{ex.backtrace.join("\n")}"
      ensure
        begin
          @@logger.debug "spawn> PID = #{Process.pid}: finish execute yield"
    ...
    @@logger.debug "spawn> parent PID = #{Process.pid} detach child process"
    # detach from child process (parent may still wait for detached process if they wish)
    Process.detach(child)
    @@logger.debug "spawn> parent PID = #{Process.pid} create spawn id"
