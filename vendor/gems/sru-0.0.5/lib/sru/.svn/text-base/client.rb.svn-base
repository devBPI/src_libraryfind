...
module SRU
  ...
  class Client
    ...
    def get_doc(hash)
      ...
      begin
        if @use_proxy
          proxy = Net::HTTP::Proxy(@host.to_s, @port);
          xml = proxy.start(uri.host.to_s){ |http| http.get(uri.path.to_s + "?" + uri.query.to_s) }
        else
        ...
        case @parser
          ...
          xmlObj.string = xml.body;
          return xmlObj.parse
          when 'rexml'
          return REXML::Document.new(xml.body)
          when 'nokogiri'
          xmlObj = Nokogiri::XML.parse(xml.body)
        end
      rescue => e
        raise SRU::Exception, "exception during SRU operation #{e.message}\n#{e.backtrace.join("\n")}", caller
      end
    end
  end
end
