  ...
  module XPath
    ...
    def xpath_all(pdoc, path, namespace = '')
      case parser_type(pdoc)
        ...
        when 'nokogiri'
        if namespace!=""
          return pdoc.xpath(path, namespace).to_a if pdoc.xpath(path, namespace)
        else
          return pdoc.xpath(path).to_a if pdoc.xpath(path)
        end
        when 'rexml'
        if namespace!=""
    ...
    def parser_type(x)
      ...
      if !x.blank? and x.class.to_s.starts_with?("Nokogiri")
        return "nokogiri"
      end
      case x.class.to_s
